...
#import "ViewControllerDocB.h"
#import "ViewControllerCircle.h"
@interface ViewControllerDocB ()
@property (nonatomic,strong) ViewControllerCircle *circleVC;
@end

@implementation ViewControllerDocB
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if([segue.identifier isEqualToString:@"DocB5Times"] )
        vcCircle.userScale=0.8;
    else if([segue.identifier isEqualToString:@"DocB3Times"] )
        vcCircle.userScale=0.5;
    else vcCircle.userScale=0.3;
    
}

- (IBAction)fiveTimesClicked:(id)sender
{
    self.circleVC.userScale=0.8;
}

- (IBAction)threeTimesClicked:(id)sender
{
